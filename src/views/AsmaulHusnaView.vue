<template>
    <MainLayout>
        <div>
            ...
            <p class="mb-2">"Sesunguhnya Allah memiliki 99 nama, seratus kurang satu,<br class="hidden md:block">siapa yang
            ...
            <div class="mt-14 md:mt-20 mb-56">
                <div class="flex flex-wrap justify-normal md:justify-around gap-7" id="list">
                    <div v-for="item in listItem" :key="item.urutan"
                        ...
                        <h5 class="text-end text-3xl font-semibold block">{{ item.arab }}</h5>
                        <span class="text-start block text-xl font-semibold mr-3">{{ item.latin }}</span>
                        <div class="text-xs">
                            <span class="text-start block font-medium">{{ item.arti }}</span>
                        </div>
                    </div>
                </div>
            </div>
